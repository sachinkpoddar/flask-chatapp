...
def login():
    ...
    if request.method == 'POST':
        ...
        if roomname in {None, ""}:
            return redirect(url_for('login', error="noroom"))
            #rooms = db.get_rooms()
            #return render_template('rooms.html', username=session[username], rooms=rooms)
        ...
        try:
            if db.validate_login(username, password):
                ...
                #return redirect(url_for('index'))
                messages = db.get_messages(roomname)
                return render_template('room.html', username=session['username'], room=roomname, messages=messages)
            else:
                return redirect(url_for('signup', error='invalidlogin'))
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if username in {None, ""}:
            return redirect(url_for('signup', error="nouser"))
        else:
